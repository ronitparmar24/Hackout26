...
import { Leaf, Lock, Mail, ArrowRight, Sparkles, ShieldCheck, AlertCircle } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authError) {
        // Fallback for dev/mock mode or invalid credentials
        if (email && password.length >= 6) {
          // Store email and session token in local storage for demonstration
    ...
    <div className="min-h-[85vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 mb-4 shadow-[0_0_25px_rgba(16,185,129,0.3)]">
            <Leaf className="w-6 h-6" />
          </div>
          <h1 className="text-2xl sm:text-3xl font-bold font-heading text-white">Welcome Back</h1>
          <p className="text-xs sm:text-sm text-white/50 mt-1">Sign in to your CarbonSense Scope 3 audit portal</p>
        </div>
        ...
        <GlassCard variant="strong" className="p-8 border-white/15 shadow-2xl relative overflow-hidden">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="email"
                  required
                  placeholder="sustainability@enterprise.com"
                  value={email}
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-xs font-semibold text-white/70 font-heading">
                  Password
                </label>
              </div>
              <div className="relative">
          ...
          <div className="space-y-2.5">
            ...
            >
              ...
              <span>Continue as Guest Demo (Live Data)</span>
            </button>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2.5 py-2.5 px-4 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-white/80 font-heading text-xs transition-colors"
            >
